...
public class EmailValidator {
    public static void main(String[] args) {
        ...
        while (!(expression = scanner.nextLine()).equals("Complete")) {
            commands = expression.split("\\s+");
            switch (commands[0]) {
                case "Make":
                    mail = makesLetters(mail, commands[1]);
                    break;
                case "GetDomain":
                    getsDomain(mail, Integer.parseInt(commands[1]));
                    break;
                case "GetUsername":
                    getsUsername(mail);
                    break;
                case "Replace":
                    mail = replacesLetter(mail, commands[1].charAt(0));
                    break;
                case "Encrypt":
                    encryptsLetters(mail);
                    break;
                default:
                    break;
            }
