package x21_ObjectsAndClasses.moreExercise;
...
public class ME04TeamworkProjects {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < number; i++) {
            ...
            TeamworkProjects.Team haveTeam = teamList.stream()
                    .filter(s -> s.getName().equals(name)).findFirst().orElse(null);
            TeamworkProjects.Team haveOwner = teamList.stream()
                    .filter(s -> s.getOwner().equals(owner)).findFirst().orElse(null);
        ...
        while (!"end of assignment".equals(input = console.nextLine())) {
            ...
            TeamworkProjects.Team haveTeam = teamList.stream()
                    .filter(s -> s.getName().equals(team)).findFirst().orElse(null);
            TeamworkProjects.Team haveMember = teamList.stream()
                    .filter(s -> s.getMembers().contains(memberToAdd) || s.getOwner().equals(memberToAdd)).findFirst().orElse(null);
        ...
        teamList.stream().filter(s -> s.getMembers().size() > 0)
                .sorted(Comparator.comparing(TeamworkProjects.Team::numberOfMembers)
                        .reversed().thenComparing(TeamworkProjects.Team::getName))
