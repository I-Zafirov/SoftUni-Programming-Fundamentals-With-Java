    ...
    public static void main(String[] args) {
        ...
        statistic.forEach((key1, value1) -> {
            System.out.printf("%s::(%.2f/%.2f/%.2f)%n", key1,
                    value1.values().stream().map(doubles -> doubles.get(0))
                            .mapToDouble(Double::doubleValue).average().orElseThrow(),
                    value1.values().stream().map(doubles -> doubles.get(1))
                            .mapToDouble(Double::doubleValue).average().orElseThrow(),
                    value1.values().stream().map(doubles -> doubles.get(2))
                            .mapToDouble(Double::doubleValue).average().orElseThrow());
